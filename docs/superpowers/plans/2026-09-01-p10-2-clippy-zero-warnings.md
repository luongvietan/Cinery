...
- [ ] **Step 1: Unused imports — delete the `use` lines**
...
- [ ] **Step 2: needless_borrow / needless_borrows_for_generic_args — remove `&`**
...
- [ ] **Step 6: one-line semantic rewrites**
...
- [ ] **Step 7: items_after_test_module**
...
- [ ] **Step 9: Commit** `refactor: clear mechanical clippy lints`
...
- [ ] **Step 1: cloned_ref_to_slice_refs** — at each site the pattern is `&vec.clone()[..]`-style; replace with `&vec[..]`. Read each call site first; confirm the clone isn't needed for ownership (it isn't — clippy proves the compiler reborrows).
- [ ] **Step 2: collapsible_if** — collapse `if a { if b { … } }` into `if a && b { … }`.
- [ ] **Step 3: ptr_arg** — change signatures and internal `path.as_path()`/`display()` calls as needed.
- [ ] **Step 4: Verify + commit** — `refactor: clear ptr_arg and collapsible lints`
...
- [ ] **Step 1b: Add accessor-parity tests** — for every public constructor/method used across the crate (`new`, `with_diagnostic`, `with_status_code`, `with_provider_message`, `with_request_id`, `with_operation`, `display_text`), assert behavior unchanged after the boxing.

- [ ] **Step 2: Introduce boxed body**
  ```rust
...
- [ ] **Step 4: Fix compile errors crate-wide**
...
- [ ] **Step 5: Handle serde transparent check** — newtype `ProviderError(Box<Body>)` serializes as the inner body automatically (Box<T> is transparent for serde). Add the serialization-parity test in Step 1b.
...
- [ ] **Step 7: Commit** `refactor: box ProviderError to satisfy result_large_err`
...
- [ ] **Step 1: Failing-size compile-proof via tests-first is not meaningful for signatures; instead write a call-site inventory test**: none — proceed directly; the gate is the lint itself.
- [ ] **Step 2: Define structs adjacent to each function; keep field order = old argument order.**
- [ ] **Step 3: Migrate call sites mechanically.** Inventory via grep before editing:
...
- [ ] **Step 4: `test_tbd` helper** — replace with a `CanonTbdRecord { id, …, ..base() }` pattern or pass a struct; test-only, keep call sites in the same file.
- [ ] **Step 5: Type aliases**
...
- [ ] **Step 6: Verify + commit** `refactor: group multi-arg APIs into request structs`

### Task 5: Full gate + regression

- [ ] **Step 1:** `cargo clippy --all-targets -- -D warnings` exits 0
- [ ] **Step 2:** `cargo fmt --all -- --check` clean (run `cargo fmt --all` first if needed)
- [ ] **Step 3:** `cargo test` (all targets) passes
- [ ] **Step 4:** Frontend untouched — no pnpm/test run required, but if any `packages/domain` or `apps/desktop/src` file changed, run `pnpm test`
- [ ] **Step 5:** Commit `gate: enforce zero-warning clippy across all targets`
- [ ] **Step 6:** Update this file's checkboxes; report lint-before/after (132 → 0) and the two structural refactors (boxed error, request structs) in the final summary
