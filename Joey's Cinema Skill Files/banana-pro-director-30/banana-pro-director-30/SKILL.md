...
- **Wardrobe:** every garment top to bottom — fabric, color, fit (cropped, oversized, fitted, baggy), structural details (cutouts, keyholes, ribbing, ribbed cotton, knit, denim wash, leather finish, mesh, latex, silk), neckline, sleeve length, hem position, layering, branding details (name the brand when known, plus the visual description of the branding itself — logo placement, wordmark, graphic treatment)
...
**Brand name rule.** Real brand names are allowed in the prompt output. Name the brand directly when the user specifies one, and pair it with the visual descriptors that actually carry the look — "white three-stripe adidas track pants," "an oversized pink Victoria's Secret PINK hoodie." The brand name sets the register; the description does the rendering work. Never rely on the brand name alone to convey a garment.

This rule does NOT extend to camera, lens, or film-stock naming in the camera spec — those stay in plain-language look terms (see the cinema-prose register), because the image tools don't parse hardware model names.
...
- Real brand names are allowed, paired with visual description
...
2. **Real brand names are allowed in prompt output.** Name the brand when the user specifies one, always paired with the visual descriptors that carry the look — never the brand name alone. Exception: camera, lens, and film-stock brands stay out of the camera spec (plain-language look terms only, since the tools don't parse hardware model names).
...
- [ ] No character names, no internal context, no meta-commentary (brand names permitted, paired with visual description)
