...
pub const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 12,
        sql: include_str!("../../migrations/0012_world_scene_pipeline.sql"),
    },
];
...
mod tests {
    ...
    fn p7_rejects_duplicate_scene_ordinal() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO scenes (id, project_id, ordinal, title, summary, created_at, updated_at) VALUES ('scene-1', 'proj-1', 1, 'Title', 'Summary', 'now', 'now')",
            [],
        )
        .unwrap();
        let dup = conn.execute(
            "INSERT INTO scenes (id, project_id, ordinal, title, summary, created_at, updated_at) VALUES ('scene-2', 'proj-1', 1, 'Other', 'Summary2', 'now', 'now')",
            [],
        );
    ...
    fn p7_rejects_duplicate_character_assignment() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO scene_characters (id, scene_id, character_entity_id, look_asset_version_id, created_at, updated_at) VALUES ('sc-1', 'scene-1', 'char-1', 'ver-1', 'now', 'now')",
            [],
        )
        .unwrap();
        let dup = conn.execute(
            "INSERT INTO scene_characters (id, scene_id, character_entity_id, look_asset_version_id, created_at, updated_at) VALUES ('sc-2', 'scene-1', 'char-1', 'ver-2', 'now', 'now')",
            [],
        );
    ...
    fn p7_rejects_invalid_reference_ids() {
        ...
        let fk1 = conn.execute(
            "INSERT INTO scene_characters (id, scene_id, character_entity_id, look_asset_version_id, created_at, updated_at) VALUES ('sc-bad', 'missing-scene', 'char-1', 'ver-1', 'now', 'now')",
            [],
        );
        assert!(fk1.is_err(), "invalid scene_id should be rejected, got {fk1:?}");
        // Invalid asset_version FK
        let fk2 = conn.execute(
            "INSERT INTO scene_characters (id, scene_id, character_entity_id, look_asset_version_id, created_at, updated_at) VALUES ('sc-bad2', 'scene-1', 'char-1', 'missing-version', 'now', 'now')",
            [],
        );
        assert!(fk2.is_err(), "invalid look_asset_version_id should be rejected, got {fk2:?}");
        // Invalid world FK
        let fk3 = conn.execute(
            "INSERT INTO scenes (id, project_id, ordinal, title, summary, world_id, created_at, updated_at) VALUES ('scene-2', 'proj-1', 2, 'Title2', 'Summary2', 'missing-world', 'now', 'now')",
            [],
        );
