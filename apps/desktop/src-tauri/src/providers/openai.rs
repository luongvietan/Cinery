...
mod tests {
    ...
    impl HttpTransport for FixtureTransport {
        fn post_json(
            ...
            Ok(
                serde_json::json!({"created": 1, "data": [{"url": "https://files.example/result.png"}], "model": "gpt-image-1"}),
            )
        }
    ...
    fn reference_adapter_translates_request_and_normalizes_immediate_completion() {
        ...
        let provider = OpenAiImageProvider::with_transport(
        ...
        assert_eq!(
            provider.fetch_result(&submission.job).unwrap().outputs[0].uri,
            "https://files.example/result.png"
        );
    }
    ...
    fn provider_errors_never_echo_bearer_token() {
        ...
        impl HttpTransport for ErrorTransport {
            fn post_json(
                ...
                Err("Authorization: Bearer secret".into())
            }
            fn get_json(&self, _: &str, _: &str) -> Result<serde_json::Value, String> {
                Err("bad".into())
            }
        ...
        let provider =
            OpenAiImageProvider::with_transport("https://api.example/v1", "secret", ErrorTransport);
...
impl OpenAiImageProvider {
    pub fn new(endpoint: impl Into<String>, bearer_token: impl Into<String>) -> Self {
        Self::with_transport(
            endpoint,
            bearer_token,
            UreqTransport::new(std::time::Duration::from_secs(60)),
        )
    }
    ...
    fn submit(
        ...
        let response = if request.reference_attachments.is_empty() {
            ...
            let mut parts = vec![
                MultipartPart {
                    ...
                    file_name: None,
                    content_type: None,
                    bytes: request.selected_model.clone().into_bytes(),
                },
                MultipartPart {
                    field_name: "prompt".into(),
                    file_name: None,
                    content_type: None,
                    bytes: request.prompt.clone().into_bytes(),
                },
                MultipartPart {
                    field_name: "input_fidelity".into(),
                    file_name: None,
                    content_type: None,
                    bytes: b"high".to_vec(),
                },
        ...
        let items = response
            ...
            .ok_or_else(|| {
                Self::error(
                    ...
                    "OpenAI response did not contain a data array",
                    response.to_string(),
                )
            })?;
        if items.is_empty() {
            return Err(Self::error(
                ProviderErrorKind::MalformedProviderResponse,
                "OpenAI response contained no generated images",
                response.to_string(),
            ));
        }
        ...
        for item in items {
            let output = normalize_output(item).ok_or_else(|| {
                Self::error(
                    ProviderErrorKind::MalformedProviderResponse,
                    "OpenAI response contained an unreadable image payload",
                    response.to_string(),
                )
            })?;
            outputs.push(output);
        ...
        self.results.lock().unwrap().insert(
            ...
            ProviderResult {
                outputs,
                provider_reported_model: response
                    .get("model")
        ...
        self.results
            ...
            .ok_or_else(|| {
                Self::error(
                    ...
                    "OpenAI image result was not found",
                    &job.provider_job_id,
                )
            })
    }
...
mod gpt_image_2_tests {
    ...
    fn without_references_posts_json_generations() {
        ...
        let provider =
            OpenAiImageProvider::with_transport("https://api.example/v1", "secret", transport);
    ...
    fn with_references_posts_multipart_edits_with_high_fidelity() {
        ...
        request
            ...
            .push(ProviderReferenceAttachment {
        ...
        let image_parts: Vec<&MultipartPart> =
            parts.iter().filter(|p| p.field_name == "image[]").collect();
        assert_eq!(
            image_parts.len(),
            2,
            "one image[] part per verified attachment"
        );
        ...
        let fidelity = parts
            .iter()
            .find(|p| p.field_name == "input_fidelity")
            .unwrap();
        ...
        assert_eq!(
            json_requests.lock().unwrap().len(),
            0,
            "references must never use the JSON generations endpoint"
        );
    }
    ...
    fn capabilities_declare_gpt_image_2_with_reference_edit_support() {
        ...
        assert!(capabilities
            .supported_models
            .contains(&"gpt-image-2".to_string()));
