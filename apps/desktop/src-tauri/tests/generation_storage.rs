...
fn materializes_project_relative_png_and_verifies_hash_and_dimensions() {
    ...
    assert_eq!(read_and_verify(root.path(), &artifact.storage_path, &artifact.sha256).unwrap(), png_bytes());
}
...
fn verification_reports_missing_and_corrupt_artifacts() {
    ...
    let missing = read_and_verify(root.path(), &artifact.storage_path, &artifact.sha256).unwrap_err();
    ...
    let corrupt = read_and_verify(root.path(), &artifact.storage_path, &artifact.sha256).unwrap_err();
