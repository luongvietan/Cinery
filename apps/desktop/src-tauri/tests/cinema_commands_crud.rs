...
fn canonical_version(root: &std::path::Path, asset_type: &str, label: &str, pixel: [u8; 4]) -> String {
...
fn fixture() -> Fixture {
    ...
    Fixture { _temp: temp, root: path.to_string_lossy().to_string() }
}
...
fn world_pin_and_shot_lifecycle_flow_through_commands() {
    let f = fixture();
    let world = canonical_version(std::path::Path::new(&f.root), "world_plate", "World", [10, 20, 30, 255]);
    let keyframe = canonical_version(std::path::Path::new(&f.root), "shot_keyframe", "KF", [40, 50, 60, 255]);
    ...
    let detail = get_scene(f.root.clone(), scene.id.clone()).unwrap();
    assert_eq!(detail.scene.world_asset_version_id.as_deref(), Some(world.as_str()));

    let shot = create_shot(f.root.clone(), scene.id.clone(), None, 4.0, "Establish".into(), None, None).unwrap();
    let updated = update_shot(f.root.clone(), shot.id.clone(), Some(6.0), Some("Close".into()), Some("lean".into()), Some("medium".into())).unwrap();
    assert_eq!(updated.duration_seconds, 6.0);

    set_shot_keyframe(f.root.clone(), shot.id.clone(), Some(keyframe.clone())).unwrap();
    let detail = get_scene(f.root.clone(), scene.id.clone()).unwrap();
    assert_eq!(detail.shots[0].keyframe_asset_version_id.as_deref(), Some(keyframe.as_str()));
    ...
    let shot2 = create_shot(f.root.clone(), scene.id.clone(), None, 4.0, "Second".into(), None, None).unwrap();
    let reordered = reorder_shots(f.root.clone(), scene.id.clone(), vec![shot2.id.clone(), shot.id.clone()]).unwrap();
    assert_eq!(reordered[0].id, shot2.id);
    assert_eq!(reordered.iter().map(|s| s.ordering).collect::<Vec<_>>(), vec![0, 1]);

    // Readiness: no cast -> blocker, stable action target.
    let readiness = get_scene_readiness(f.root.clone(), scene.id.clone()).unwrap();
    assert!(!readiness.ready);
    assert!(readiness.blockers.iter().any(|b| b.code == "missing_cast_look" && b.action_target == "cast"));
...
fn invalid_mutations_return_stable_error_codes_not_panics() {
    ...
    let error = create_shot(f.root.clone(), scene.id.clone(), None, 99.0, "Too long".into(), None, None).unwrap_err();
    ...
    let asset = AssetService::create_asset(std::path::Path::new(&f.root), "world_plate", "W", None).unwrap();
    let source = image_file(std::path::Path::new(&f.root), "w.png", [1, 2, 3, 255]);
    let candidate = AssetService::import_asset_version(std::path::Path::new(&f.root), &asset.id, &source, None).unwrap();
    let error = set_scene_world(f.root.clone(), scene.id.clone(), Some(candidate.id.clone())).unwrap_err();
