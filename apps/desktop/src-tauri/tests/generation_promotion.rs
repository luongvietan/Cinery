...
fn promotion_rejects_wrong_asset_type_and_owner() {
    ...
    let artifacts = capture(&root);
    let type_error = GenerationService::promote_generated_artifact(&root, &artifacts[0].id, "wrong-type", false).unwrap_err();
    assert!(
        matches!(type_error, AppError::GenerationArtifactNotPromotable),
        "wrong asset type must be rejected, got {type_error:?}"
    );
    let owner_error = GenerationService::promote_generated_artifact(&root, &artifacts[0].id, "wrong-owner", false).unwrap_err();
    assert!(
...
fn promotion_accepts_matching_character_owned_face_target() {
    ...
    let artifacts = capture(&root);
    let version = GenerationService::promote_generated_artifact(&root, &artifacts[0].id, "mara-face", true).unwrap();
    let asset = AssetService::get_asset_with_versions(&root, "mara-face").unwrap();
    assert_eq!(asset.asset.canonical_version_id.as_deref(), Some(version.id.as_str()));
}
...
fn promotion_rejects_target_in_another_project() {
    ...
    let other_asset = AssetService::create_asset(&other_root, "face_lock", "Other Face", None).unwrap();

    let artifacts = capture(&root);
    let error = GenerationService::promote_generated_artifact(&root, &artifacts[0].id, &other_asset.id, false).unwrap_err();
    assert!(
        matches!(error, AppError::GenerationProjectMismatch | AppError::AssetNotFound | AppError::GenerationArtifactNotPromotable),
