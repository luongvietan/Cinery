    ...
    fn input(&self, provider_id: &str) -> Value {
        json!({
            ...
            "qaMockResponse": {
                "schemaVersion": 1,
                "checks": [
                    {"checkId":"artifact:unexpected","status":"pass","confidence":0.99,"observed":"clean","reason":"fixture","repairHint":null},
                    {"checkId":"artifact:watermark","status":"pass","confidence":0.99,"observed":"clean","reason":"fixture","repairHint":null},
                    {"checkId":"lock:right_eyebrow_scar","status":"pass","confidence":0.99,"observed":"correct","reason":"fixture","repairHint":null},
                    {"checkId":"reference:identity","status":"pass","confidence":0.99,"observed":"same","reason":"fixture","repairHint":null},
                    {"checkId":"reference:look","status":"pass","confidence":0.99,"observed":"same","reason":"fixture","repairHint":null}
                ],
                "modelSummary":"repaired"
            }
        })
    }
}
...
fn repair_waits_for_approval_and_creates_one_child_with_full_provenance() {
    ...
    let child_qa: (String, String) = conn
        .query_row(
            ...
            params![child_qa_id, auto_qa_workflow_id],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .unwrap();
    assert_eq!(child_qa, ("succeeded".into(), "pass".into()));
}
