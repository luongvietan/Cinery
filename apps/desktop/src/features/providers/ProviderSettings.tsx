...
export function ProviderSettings({ projectRootPath }: { projectRootPath: string }) {
  ...
  return <section className="provider-settings" aria-labelledby="provider-settings-title">
    <header className="workflow-panel-header"><div><h2 id="provider-settings-title">Connect an AI service</h2><p>Cinery generates images and video through an AI service you connect, such as an image API you already have a key for. You need the service's web address (Base URL), a model name, and your API key. Keys are stored securely in your operating system's credential vault, never in the project folder.</p></div></header>
    {error ? <p role="alert">{error}</p> : null}{message ? <p role="status">{message}</p> : null}
    <div className="workflow-form-actions"><button type="button" onClick={() => { operationRef.current += 1; selectionRef.current = ""; setSelectedId(""); setDraft(emptyProvider()); setError(null); setMessage(null); setTesting(false); }}>Add a service</button></div>
    {providers.length ? <label>Saved providers<select aria-label="Saved providers" value={selectedId} onChange={(event) => selectProvider(event.target.value)}><option value="">Select a provider</option>{providers.map((provider) => <option key={provider.providerId} value={provider.providerId}>{provider.displayName} ({provider.purpose})</option>)}</select></label> : <p>No AI service connected yet. Fill in the form below to add one.</p>}
    ...
    <label>Provider ID<input value={draft.providerId} onChange={(event) => updateDraft("providerId", event.target.value)} placeholder="my-image-provider" aria-describedby="provider-id-help" /><span id="provider-id-help" className="workflow-field-help">A short internal name, lowercase letters and hyphens. It never appears in your generated work.</span></label>
