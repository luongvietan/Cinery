...
function artifactFormatLabel(mimeType: string): string {
    ...
    <article className={selected ? "generation-result-card generation-result-card--selected" : "generation-result-card"}>
      <button type="button" className="generation-result-select" aria-pressed={selected} aria-label={`Select result ${artifact.ordinal}`} onClick={onSelect}>
        {unavailable ? (
          ...
          // Never autoplays: the user presses play explicitly. preload
          // "metadata" keeps the list cheap for large MP4s.
          <video src={convertFileSrc(source)} controls preload="metadata" playsInline muted aria-label={`Generated video ${artifact.ordinal}`} />
        ) : (
        ...
        <span>{isVideo ? "Video" : <>{artifact.width ?? "—"} × {artifact.height ?? "—"} · {artifactFormatLabel(artifact.mimeType)}</>}</span>
