...
export function ProductionWorkspace({ projectRootPath }: { projectRootPath: string }) {
  ...
  async function handleCreate(input: Record<string, unknown>) { setPending(true); setError(null); try { const created = await createWorkflowRun(projectRootPath, input); const waiting = await advanceWorkflowRun(projectRootPath, created.run.id); setSelectedRun(waiting); setShowCreate(false); setRuns(await listWorkflowRuns(projectRootPath)); } catch (reason) { setError(describeError(reason)); } finally { setPending(false); } }
